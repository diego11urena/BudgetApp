...
export type OnboardingStep = "income" | "expenses" | "accounts" | "savings";
...
export interface OnboardingState {
  ...
  hasAccounts: boolean;
  hasSavings: boolean;
  /** The first step not yet done, or null if all four are done. */
  nextStep: OnboardingStep | null;
}

/**
 * Resolves onboarding progress. Income and Accounts still require at least
 * one row, so row presence is a valid completion signal for them. Expenses
 * and Savings allow zero rows, so they're tracked via explicit
...
export async function getOnboardingState(userId: string): Promise<OnboardingState> {
  ...
  const [incomeCount, balanceCount] = await Promise.all([
  ...
  return { cycle, hasIncome, hasExpenses, hasAccounts, hasSavings, nextStep };
}
